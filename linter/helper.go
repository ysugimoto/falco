...
// set var.Bar = (req.http.Host "example.com");    // -> invalid, right expression must be a STRING constant
...
//
// So we'd check validity following function.
func isValidStatementExpression(left types.Type, exp ast.Expression) error {
	switch t := exp.(type) {
	case *ast.PrefixExpression:
		if t.Operator == "!" {
	...
	case *ast.GroupedExpression:
		if left == types.StringType {
			return fmt.Errorf("could not specify grouped expression if left type is STRING")
		}
	}
	return nil
}
