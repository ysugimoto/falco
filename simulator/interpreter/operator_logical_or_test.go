...
func TestLogicalOrOperator(t *testing.T) {
	t.Run("left is INTEGER", func(t *testing.T) {
		...
		for i, tt := range tests {
			ip := New(nil)
			v, err := ip.ProcessLogicalOrOperator(tt.left, tt.right)
			if tt.isError {
				if err == nil {
	...
	t.Run("left is BOOL", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: &variable.Boolean{Value: true}, right: &variable.Boolean{Value: false}, expect: true},
			{left: &variable.Boolean{Value: true}, right: &variable.Boolean{Value: false, Literal: true}, expect: true},
			{left: &variable.Boolean{Value: true}, right: &variable.IP{Value: net.ParseIP("127.0.0.1")}, isError: true},
			{left: &variable.Boolean{Value: true, Literal: true}, right: &variable.Boolean{Value: false}, expect: true},
			{left: &variable.Boolean{Value: true, Literal: true}, right: &variable.Boolean{Value: false, Literal: true}, expect: true},
		}
