...
func TestProcessLeftShift(t *testing.T) {
	t.Run("left is INTEGER", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: 10, right: &variable.Integer{Value: 1}, expect: 20},
			{left: 10, right: &variable.Integer{Value: 1, Literal: true}, expect: 20},
		...
		for i, tt := range tests {
			...
			err := ip.ProcessLeftShiftAssignment(left, tt.right)
			if tt.isError {
				if err == nil {
...
func TestProcessRightShift(t *testing.T) {
	t.Run("left is INTEGER", func(t *testing.T) {
		now := time.Now()
		tests := []struct {
			left    int64
			...
			{left: 10, right: &variable.Integer{Value: 100}, expect: 0},
			{left: 10, right: &variable.Integer{Value: 100, Literal: true}, expect: 0},
			{left: 10, right: &variable.Float{Value: 50.0}, isError: true},
			{left: 10, right: &variable.Float{Value: 50.0, Literal: true}, isError: true},
			{left: 10, right: &variable.String{Value: "example"}, isError: true},
			{left: 10, right: &variable.String{Value: "example", Literal: true}, isError: true},
		...
		for i, tt := range tests {
			ip := New(nil)
			left := &variable.Integer{Value: tt.left}
			err := ip.ProcessRightShiftAssignment(left, tt.right)
			if tt.isError {
				if err == nil {
					t.Errorf("Index %d: expects error but non-nil", i)
				}
