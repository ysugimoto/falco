...
func TestProcessSubtraction(t *testing.T) {
	t.Run("left is INTEGER", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: 10, right: &variable.Integer{Value: 100}, expect: -90},
			{left: 10, right: &variable.Integer{Value: 100, Literal: true}, expect: -90},
			{left: 10, right: &variable.Float{Value: 50.0}, expect: -40.0},
			...
			{left: 10, right: &variable.RTime{Value: 100 * time.Second}, expect: -90},
		...
		for i, tt := range tests {
			...
			err := ip.ProcessSubtractionAssignment(left, tt.right)
			if tt.isError {
				if err == nil {
	...
	t.Run("left is FLOAT", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: 10.0, right: &variable.Integer{Value: 100}, expect: -90.0},
			{left: 10.0, right: &variable.Integer{Value: 100, Literal: true}, expect: -90.0},
			{left: 10, right: &variable.Float{Value: 50.0}, expect: -40.0},
			{left: 10, right: &variable.Float{Value: 50.0, Literal: true}, expect: -40.0},
			{left: 10, right: &variable.String{Value: "example"}, isError: true},
			{left: 10, right: &variable.String{Value: "example", Literal: true}, isError: true},
	...
	t.Run("left is RTIME", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: time.Second, right: &variable.Integer{Value: 100}, expect: -99 * time.Second},
			{left: time.Second, right: &variable.Integer{Value: 100, Literal: true}, isError: true},
			{left: time.Second, right: &variable.Float{Value: 50.0}, expect: -49 * time.Second},
			...
			{left: time.Second, right: &variable.RTime{Value: 100 * time.Second}, expect: -99 * time.Second},
			{left: time.Second, right: &variable.RTime{Value: 100 * time.Second, Literal: true}, expect: -99 * time.Second},
