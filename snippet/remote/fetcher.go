...
func (f *FastlyApiFetcher) ResponseObjects() ([]*snippet.ResponseObject, error) {
	...
	// Fastly implicit spec:
	// Enabled console snippets like IP Block List, synthetic responses are embedded after the user's custom response objects.
	// There is no way to distinguish them, so we will judge that the name starts with "Geneted by" string.
	sort.Slice(responseObjects, func(i, j int) bool {
		return strings.HasPrefix(responseObjects[j].Name, "Generated by")
	})
