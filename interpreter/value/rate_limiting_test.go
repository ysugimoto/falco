...
func TestCalculateBucket(t *testing.T) {
	// 2023-03-15 13:20:08.000 UTC
	fixedTime := int64(1678886408000)

	entries := []rateEntry{
		{Count: 1, Timestamp: fixedTime - 5000},  // 1678886403000
		{Count: 1, Timestamp: fixedTime - 15000}, // 1678886393000
		{Count: 1, Timestamp: fixedTime - 25000}, // 1678886383000
	}

	// 10s bucket: from 1678886400000 to 1678886410000
	// only the last entry is included
	if val := calculateBucketWithTime(fixedTime, entries, 10*time.Second); val != 1 {
		t.Errorf("bucket(10s) expected 1, got %d", val)
	}

	// 20s window: from 1678886390000 to 1678886410000
	// last 2 entries are included
	if val := calculateBucketWithTime(fixedTime, entries, 20*time.Second); val != 2 {
		t.Errorf("bucket(20s) expected 2, got %d", val)
	}

	// 30s window: from 1678886380000 to 1678886410000
	// all 3 entries are included
...
func TestRatecounter(t *testing.T) {
	...
	if *rc.LastIncremented != client {
		t.Errorf("Expected LastIncremented to be initialized with string value after Increment, but was false")
	}
