...
func TestProcessMultiplication(t *testing.T) {
	t.Run("left is INTEGER", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: 10, right: &value.Integer{Value: 100}, expect: 1000},
			{left: 10, right: &value.Integer{Value: 100, Literal: true}, expect: 1000},
			{left: 10, right: &value.Float{Value: 50.0}, expect: 500},
		...
		for i, tt := range tests {
			...
			err := Multiplication(left, tt.right)
			if tt.isError {
				if err == nil {
	...
	t.Run("left is FLOAT", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: 10.0, right: &value.Integer{Value: 100}, expect: 1000.0},
			{left: 10.0, right: &value.Integer{Value: 100, Literal: true}, expect: 1000.0},
			{left: 10.0, right: &value.Float{Value: 50.0}, expect: 500.0},
			{left: 10.0, right: &value.Float{Value: 50.0, Literal: true}, expect: 500.0},
	...
	t.Run("left is RTIME", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: time.Second, right: &value.Integer{Value: 100}, expect: 100 * time.Second},
			{left: time.Second, right: &value.Integer{Value: 100, Literal: true}, expect: 100 * time.Second},
			{left: time.Second, right: &value.Float{Value: 50.0}, expect: 50 * time.Second},
			{left: time.Second, right: &value.Float{Value: 50.0, Literal: true}, expect: 50 * time.Second},
