...
func TestNotEqualOperator(t *testing.T) {
	...
	t.Run("left is STRING", func(t *testing.T) {
		...
		tests := []struct {
			...
			{left: &value.String{Value: "some_backend"}, right: &value.Backend{Value: &ast.BackendDeclaration{Name: &ast.Ident{Value: "some_backend"}}}, expect: false},
			...
			{left: &value.String{Value: nowStr}, right: &value.Time{Value: now}, expect: false},
			{left: &value.String{Value: "example"}, right: &value.Time{Value: now}, expect: true},
			...
			{left: &value.String{Value: "$unsatisfiable"}, right: &value.Regex{Unsatisfiable: true}, expect: false},
		}
