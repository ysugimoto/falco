...
// Fastly built-in function testing implementation of math.roundhalfup
// Arguments may be:
// - FLOAT
// Reference: https://developer.fastly.com/reference/vcl/functions/math-rounding/math-roundhalfup/
func Test_Math_roundhalfup(t *testing.T) {
	tests := []struct {
		input  *value.Float
		expect *value.Float
		err    *value.String
		...
		{input: &value.Float{Value: 0.8}, expect: &value.Float{Value: 1}, err: nil},
		{input: &value.Float{Value: 0.2}, expect: &value.Float{Value: 0}, err: nil},
		{input: &value.Float{Value: 1.5}, expect: &value.Float{Value: 2}, err: nil},
	}

	for i, tt := range tests {
		ret, err := Math_roundhalfup(&context.Context{}, tt.input)
		if err != nil {
