...
func Math_is_infinite_Validate(args []value.Value) error {
	if len(args) != 1 {
		return errors.ArgumentNotEnough("math.is_infinite", 1, args)
	}
	for i := range args {
		if args[i].Type() != Math_is_infinite_ArgumentTypes[i] {
			return errors.TypeMismatch("math.is_infinite", i+1, Math_is_infinite_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
...
func Math_is_infinite(ctx *context.Context, args ...value.Value) (value.Value, error) {
	...
	return value.Null, errors.NotImplemented("math.is_infinite")
}
