...
func Test_Math_is_normal(t *testing.T) {
	...
	tests := []struct {
		...
		{input: &value.Float{IsNegativeInf: true}, expect: &value.Boolean{Value: true}, err: nil},
		{input: &value.Float{IsPositiveInf: true}, expect: &value.Boolean{Value: true}, err: nil},
		{input: &value.Float{Value: 1.2}, expect: &value.Boolean{Value: true}, err: nil},
		{input: &value.Float{Value: subnormalValue}, expect: &value.Boolean{Value: false}, err: nil},
	}

	for i, tt := range tests {
		ret, err := Math_is_normal(&context.Context{}, tt.input)
		if err != nil {
