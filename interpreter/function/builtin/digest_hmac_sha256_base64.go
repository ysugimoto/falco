...
func Digest_hmac_sha256_base64_Validate(args []value.Value) error {
	if len(args) != 2 {
		return errors.ArgumentNotEnough("digest.hmac_sha256_base64", 2, args)
	}
	for i := range args {
		if args[i].Type() != Digest_hmac_sha256_base64_ArgumentTypes[i] {
			return errors.TypeMismatch("digest.hmac_sha256_base64", i+1, Digest_hmac_sha256_base64_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
