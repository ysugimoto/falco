...
func Test_Math_atan(t *testing.T) {
	...
	tests := []struct {
		...
		{input: &value.Float{IsNegativeInf: true}, expect: &value.Float{Value: -math.Pi / 2}, err: nil},
		{input: &value.Float{IsPositiveInf: true}, expect: &value.Float{Value: math.Pi / 2}, err: nil},
		{input: &value.Float{Value: subnormalValue}, expect: &value.Float{Value: subnormalValue}, err: &value.String{Value: "ERANGE"}},
		{input: &value.Float{Value: 0.5}, expect: &value.Float{Value: 0.4636476090008061}, err: nil},
	}

	for i, tt := range tests {
		ret, err := Math_atan(&context.Context{}, tt.input)
		if err != nil {
