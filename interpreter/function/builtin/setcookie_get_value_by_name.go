...
func Setcookie_get_value_by_name_Validate(args []value.Value) error {
	if len(args) != 2 {
		return errors.ArgumentNotEnough("setcookie.get_value_by_name", 2, args)
	}
	for i := range args {
		if args[i].Type() != Setcookie_get_value_by_name_ArgumentTypes[i] {
			return errors.TypeMismatch("setcookie.get_value_by_name", i+1, Setcookie_get_value_by_name_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
