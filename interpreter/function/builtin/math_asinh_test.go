...
func Test_Math_asinh(t *testing.T) {
	...
	tests := []struct {
		...
		{input: &value.Float{IsPositiveInf: true}, expect: &value.Float{Value: 0}, err: nil},
		{input: &value.Float{Value: subnormalValue}, expect: &value.Float{Value: subnormalValue}, err: &value.String{Value: "ERANGE"}},
		{input: &value.Float{Value: 0.5}, expect: &value.Float{Value: 0.48121182505960347}, err: nil},
	}

	for i, tt := range tests {
		ret, err := Math_asinh(&context.Context{}, tt.input)
		if err != nil {
