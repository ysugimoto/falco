...
func Accept_media_lookup_Validate(args []value.Value) error {
	if len(args) != 4 {
		return errors.ArgumentNotEnough("accept.media_lookup", 4, args)
	}
	for i := range args {
		if args[i].Type() != Accept_media_lookup_ArgumentTypes[i] {
			return errors.TypeMismatch("accept.media_lookup", i+1, Accept_media_lookup_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
...
func Accept_media_lookup(ctx *context.Context, args ...value.Value) (value.Value, error) {
	...
	return value.Null, errors.NotImplemented("accept.media_lookup")
}
