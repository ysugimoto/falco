...
// Fastly built-in function testing implementation of math.tanh
// Arguments may be:
// - FLOAT
// Reference: https://developer.fastly.com/reference/vcl/functions/math-trig/math-tanh/
func Test_Math_tanh(t *testing.T) {
	...
	tests := []struct {
		...
		{input: &value.Float{IsNegativeInf: true}, expect: &value.Float{Value: -1}, err: &value.String{Value: "EDOM"}},
		{input: &value.Float{IsPositiveInf: true}, expect: &value.Float{Value: 1}, err: &value.String{Value: "EDOM"}},
		...
		{input: &value.Float{Value: 0.5}, expect: &value.Float{Value: 0.46211715726000974}, err: nil},
	}

	for i, tt := range tests {
		ret, err := Math_tanh(&context.Context{}, tt.input)
		if err != nil {
