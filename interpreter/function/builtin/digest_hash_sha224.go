...
func Digest_hash_sha224_Validate(args []value.Value) error {
	if len(args) != 1 {
		return errors.ArgumentNotEnough("digest.hash_sha224", 1, args)
	}
	for i := range args {
		if args[i].Type() != Digest_hash_sha224_ArgumentTypes[i] {
			return errors.TypeMismatch("digest.hash_sha224", i+1, Digest_hash_sha224_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
...
func Digest_hash_sha224(ctx *context.Context, args ...value.Value) (value.Value, error) {
	...
	return value.Null, errors.NotImplemented("digest.hash_sha224")
}
