...
func Math_is_subnormal_Validate(args []value.Value) error {
	if len(args) != 1 {
		return errors.ArgumentNotEnough("math.is_subnormal", 1, args)
	}
	for i := range args {
		if args[i].Type() != Math_is_subnormal_ArgumentTypes[i] {
			return errors.TypeMismatch("math.is_subnormal", i+1, Math_is_subnormal_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
...
func Math_is_subnormal(ctx *context.Context, args ...value.Value) (value.Value, error) {
	...
	return value.Null, errors.NotImplemented("math.is_subnormal")
}
