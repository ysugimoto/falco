...
func Digest_time_hmac_sha512_Validate(args []value.Value) error {
	if len(args) != 3 {
		return errors.ArgumentNotEnough("digest.time_hmac_sha512", 3, args)
	}
	for i := range args {
		if args[i].Type() != Digest_time_hmac_sha512_ArgumentTypes[i] {
			return errors.TypeMismatch("digest.time_hmac_sha512", i+1, Digest_time_hmac_sha512_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
