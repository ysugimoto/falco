...
sub vcl_fetch {
  ...
  set beresp.http.Fastly-Debug-Path = "(F " server.identity " " now.sec ") "
                                      if(beresp.http.Fastly-Debug-Path, beresp.http.Fastly-Debug-Path, "");
  ...
  if (!var.fastly_disable_restart_on_error) {
    if (
      (beresp.status == 500 || beresp.status == 503) && req.restarts < 1 &&
  ...
  if (
    beresp.http.Expires || beresp.http.Surrogate-Control ~ "max-age" ||
...
sub vcl_deliver {
  ...
  if (req.http.Fastly-Debug || req.http.Fastly-FF) {
    ...
    set resp.http.Fastly-Debug-TTL = if(obj.hits > 0, "(H ", "(M ") server.identity
  ...
  # add or append X-Served-By/X-Cache(-Hits)
  {
    ...
    set resp.http.X-Cache = if(resp.http.X-Cache, resp.http.X-Cache ", ", "")
                            if(fastly_info.state ~ "HIT(?:-|\z)", "HIT", "MISS");
